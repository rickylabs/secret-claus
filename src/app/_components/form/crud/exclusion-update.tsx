...
export async function ExclusionList({ pairing, pairings, disabled }: ExclusionUpdateProps) {
    const { event_id, giver } = pairing
    const { data: exclusions } = await fetchExclusionByGiverId(pairing?.event_id, pairing.giver_id)
    ...
    async function handleUpdate(list: string [], isBidirectional?: boolean) {
        ...
        const updateList = await updateExclusion(event_id, giver.id, list, isBidirectional)
        if(updateList) {
            toast({
                variant: "informative",
